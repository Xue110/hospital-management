...
import { deleteAppointment, deleteAppointmentBatch } from '../../../apis/hospital';
...
const TableComponent = (props: any) => {
  ...
  const handleDelete = async (record: any) => {
    const res = await deleteAppointment(record.id);
    if (res.code === 200) {
  ...
  const handleBatchDelete = async () => {
    const res = await deleteAppointmentBatch(selectedRowIds);
    if (res.code === 200) {
      message.success('删除成功');
      if (
